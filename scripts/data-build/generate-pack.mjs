...
async function main() {
  ...
  // Keep one connected pedestrian network so every searchable landmark can be routed to every other one.
  const adjacency = new Map(nodes.map((node) => [node.id, []])); for (const edge of edges) { adjacency.get(edge.from).push(edge.to); adjacency.get(edge.to).push(edge.from); }
  const component = new Map(); let componentId = 0; for (const node of nodes) if (!component.has(node.id)) { const pending = [node.id]; component.set(node.id, componentId); while (pending.length) { const current = pending.pop(); for (const neighbour of adjacency.get(current) ?? []) if (!component.has(neighbour)) { component.set(neighbour, componentId); pending.push(neighbour); } } componentId += 1; }
  const sizes = new Map(); for (const id of component.values()) sizes.set(id, (sizes.get(id) ?? 0) + 1); const largestComponent = [...sizes.entries()].sort((a, b) => b[1] - a[1])[0][0]; const routeNodes = nodes.filter((node) => component.get(node.id) === largestComponent); const routeEdges = edges.filter((edge) => component.get(edge.from) === largestComponent && component.get(edge.to) === largestComponent);
  ...
  const connectedPois = pois.filter((poi) => component.get(poi.nodeId) === largestComponent);
  if (!connectedPois.length) throw new Error("No official campus places could be snapped to the connected OSM walking network.");
  await mkdir(OUT, { recursive: true }); const files = [{ id: "pmtiles", filename: "lasu-ojo.pmtiles", content: archive, contentType: "application/vnd.pmtiles" }, { id: "graph", filename: "lasu-ojo-graph.json", content: text.encode(JSON.stringify({ nodes: routeNodes, edges: routeEdges })), contentType: "application/json" }, { id: "pois", filename: "lasu-ojo-pois.json", content: text.encode(JSON.stringify(connectedPois)), contentType: "application/json" }];
  ...
  const version = `${new Date().toISOString().slice(0, 10)}-${createHash("sha256").update(manifestFiles.map((file) => file.sha256).join("")).digest("hex").slice(0, 8)}`; await writeFile(resolve("public", "version.json"), JSON.stringify({ appVersion: "0.1.0", latestPack: { packId: "lasu-ojo-campus", version, releasedAt: new Date().toISOString(), bounds, releaseNotes: `Official ArcGIS campus features merged with ${ways.length} OSM walkable ways.`, attribution: ["Campus features © Lagos State University ArcGIS web map", "© OpenStreetMap contributors"], files: manifestFiles } }, null, 2));
  console.log(`Generated ${archive.length.toLocaleString()} byte PMTiles archive, ${connectedPois.length} searchable official places, ${routeNodes.length} routing nodes, and ${routeEdges.length} walking edges.`);
}
